  ...
  >
    ...
    >
      <div class="p-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Edit Project</h2>
...
const props = defineProps<{ modelValue: boolean; project: any | null }>();
...
const defaultForm = (src: any) => ({
  ...
  surveyor: {
    name: src?.surveyor?.name ?? "",
    license_no: src?.surveyor?.license_no ?? "",
  },
});

const localForm = ref(defaultForm((props as any).project));

watch(
  () => (props as any).project,
  (p) => {
    localForm.value = defaultForm(p);
  }
);
...
async function submit() {
  ...
  const id = (props as any).project?.id || (props as any).project?.project_id;
  if (!id) {
    toast.add({ title: "Missing project id.", color: "error" });
    return;
  }
  ...
  try {
    ...
    const res = await axios.put(
      `${config.public.BASE_URL}/project/edit/${id}`,
      payload
    );
